	...
	tests := map[string]struct {
		...
		"CreateHTML": {
			args: args{
				...
				r: func() *http.Request {
					req := httptest.NewRequest(http.MethodPost, "/", strings.NewReader("description=first&title=title"))
					req.Header.Set("Content-Type", "application/x-www-form-urlencoded")
					return req
				}(),
			},
			mock: func(f fields) {
				f.service.EXPECT().Add(context.Background(), "title", "first").Return(todo, nil)
			},
...
func Test_handler_Search(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"SearchHTMX": {
			...
			mock: func(f fields) {
				// Avoid using mock.AnythingOfType for the context argument in the expectation.
				// The actual context passed to the Search method will be of a different type,
				// as it's obtained using http.Request.Context(). This could cause the mock expectation to fail.
				// Use mock.Anything to allow any type of context.
				f.service.EXPECT().Search(mock.AnythingOfType("*context.emptyCtx"), "test").Return([]*domain.Todo{todo}, nil)

				// In "SearchHTML" and "SearchHTMX" test cases
				//f.service.EXPECT().Search(mock.Anything, "test").Return([]*domain.Todo{todo}, nil)
			},
			wantStatusCode: http.StatusOK,
			wantHeader: http.Header{
				"Content-Type": []string{"text/plain; charset=utf-8"},
	...
	tests := map[string]struct {
		...
		"UpdateHTML": {
			args: args{
				w: httptest.NewRecorder(),
				r: func() *http.Request {
					req := httptest.NewRequest(http.MethodPost, "/", strings.NewReader("title=title&description=description&completed=true"))
					req.Header.Set("Content-Type", "application/x-www-form-urlencoded")
					rCtx := chi.NewRouteContext()
					rCtx.URLParams.Add("todoId", todoID.String())
			...
			mock: func(f fields) {
				f.service.EXPECT().Update(mock.AnythingOfType("*context.valueCtx"), todoID, true, "title", "description").Return(updated, nil)
			},
			wantStatusCode: http.StatusFound,
			wantHeader: http.Header{
				"Location": []string{"/"},
		...
		"UpdateHTMX": {
			args: args{
				w: httptest.NewRecorder(),
				r: func() *http.Request {
					req := httptest.NewRequest(http.MethodPost, "/", strings.NewReader("title=title&description=first&completed=true"))
					req.Header.Set("Content-Type", "application/x-www-form-urlencoded")
					req.Header.Set("HX-Request", "true")
					rCtx := chi.NewRouteContext()
					rCtx.URLParams.Add("todoId", todoID.String())
			...
			mock: func(f fields) {
				f.service.EXPECT().Update(mock.AnythingOfType("*context.valueCtx"), todoID, true, "title", "first").Return(updated, nil)
			},
			wantStatusCode: http.StatusOK,
			wantHeader: http.Header{
				"Content-Type": []string{"text/html; charset=utf-8"},
