...
func TestTodos_Add(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"AddNonEmpty": {
			...
			args: args{title: "title2", description: "test2"},
			want: &Todo{
				ID:          uuid.New(),
				Title:       "title2",
				Description: "test2",
	...
	for name, tt := range tests {
		t.Run(name, func(t *testing.T) {
			...
			got := tt.l.Add(tt.args.title, tt.args.description)

			if got.ID == uuid.Nil {
				t.Errorf("todo.ID is nil")
			}
			if got.Title != tt.want.Title {
				t.Errorf("todo.Title = %v, want %v", got.Title, tt.want.Title)
			}
			if got.Description != tt.want.Description {
				t.Errorf("todo.Description = %v, want %v", got.Description, tt.want.Description)
			}
			if got.Completed != tt.want.Completed {
				t.Errorf("todo.Completed = %v, want %v", got.Completed, tt.want.Completed)
			}
			if got.CreatedAt.IsZero() {
				t.Errorf("todo.CreatedAt is zero")
			}
			if len(tt.l) != initialLength+1 {
				t.Errorf("len(todos) = %v, want %v", len(tt.l), initialLength+1)
			}
			if tt.l[initialLength] != got {
				t.Errorf("todos[%v] = %v, want %v", initialLength, tt.l[initialLength], got)
			}
		})
	}
}
...
func TestTodos_Search(t *testing.T) {
	var firstID = uuid.New()
	var first = &Todo{ID: firstID, Title: "first"}
	var secondID = uuid.New()
	var second = &Todo{ID: secondID, Title: "second"}
	var thirdID = uuid.New()
	var third = &Todo{ID: thirdID, Title: "third"}
	var fourthID = uuid.New()
	var fourth = &Todo{ID: fourthID, Title: "fourth"}
...
func TestTodos_Update(t *testing.T) {
	...
	for name, tt := range tests {
		t.Run(name, func(t *testing.T) {
			if got := tt.l.Update(tt.args.id, tt.args.completed, tt.args.title, tt.args.description); !reflect.DeepEqual(got, tt.want) {
