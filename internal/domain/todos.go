...
func (l *Todos) Add(title string, description string) *Todo {
...
func (l *Todos) Update(id uuid.UUID, completed bool, title, description string) *Todo {
	index := l.indexOf(id)
	if index == -1 {
		return nil
	}
	todo := (*l)[index]
	todo.Update(completed, title, description)
